...
// 知前序和后序求中序的个数**********************
unll res = 1;
void getInOrder(int st_p, int ed_p, int st_s, int ed_s, Node*& p) {
	...
	if (fir[st_p + 1] == bck[ed_s - 1]) {//根节点只有一个子节点 
		res *= 2;
		//去掉根节点后得到新的前序遍历和后序遍历 
		getInOrder(st_p + 1, ed_p, st_s, ed_s - 1, p->l);
	}
	else {//根节点有两个子节点
		//找到左孩子在后序遍历中的位置 
		rep(i, 2, ed_p - st_p)
			if (fir[st_p + 1] == bck[ed_s - i]) {
				//左子树的前序遍历和后序遍历 
				getInOrder(st_p + 1, ed_p - i + 1, st_s, ed_s - i, p->l);
				//右子树的前序遍历和后序遍历 
				getInOrder(ed_p - i + 2, ed_p, ed_s - i + 1, ed_s - 1, p->r);
				return;
			}
	}
...
void solve() {
	...
	//cin >> fir >> bck;
	//getInOrder(0, fir.length() - 1, 0, bck.length() - 1, root);
	//cout << res << '\n';
